import { appendAudit, dataDir, listBookIds, persistOn, readBookMeta, runWithBook, writeBookMeta } from "./nas-fs.server-P6o-d4JV.mjs";
import { Z as ALL_PERMS, et as PRESETS, m as hashPassword, nt as canWriteLedger, pt as uid, rt as hasPerm } from "./router-BQbQbUY2.mjs";
import { existsSync } from "node:fs";
import { join } from "node:path";
import { mkdir, readFile, writeFile } from "node:fs/promises";
//#region node_modules/.nitro/vite/services/ssr/assets/accounts.server-DH_I0P04.js
...
async function handleAuthPost(request) {
	...
	if (op === "deleteBook") {
		...
		await writeFileShape(data);
		const { removeBookDir } = await import("./nas-fs.server-P6o-d4JV.mjs");
