import { appendAudit, dataDir, listBookIds, persistOn, readBookMeta, runWithBook, writeBookMeta } from "./nas-fs.server-huEdTgug.mjs";
import { at as canWriteLedger, et as ALL_PERMS, gt as uid, m as hashPassword, ot as hasPerm, rt as PRESETS } from "./router-DxdzlCp3.mjs";
import { existsSync } from "node:fs";
import { join } from "node:path";
import { mkdir, readFile, writeFile } from "node:fs/promises";
//#region node_modules/.nitro/vite/services/ssr/assets/accounts.server-DNyKCx-M.js
...
async function handleAuthPost(request) {
	...
	if (op === "deleteBook") {
		...
		await writeFileShape(data);
		const { removeBookDir } = await import("./nas-fs.server-huEdTgug.mjs");
...
//#endregion
export { ensureAccounts, handleAuthPost, memberList, publicUser, resolveTenant, withTenant };
