...
import { b as nameKey } from "./contracts-CNGvTFF_.js";
import { B as mergeExpenses, I as findDuplicatePayments, N as duplicateNotice, Q as planAttendanceImport, S as parseAttendanceSheet, T as parsePeopleSheet, V as mergePayments, a as parseContractWorkbook, d as parseExpenseSheet, h as parsePaymentSheet, n as parseFullAttendanceWorkbook, s as parseInsuranceMembersSheet } from "./excel-DiKoHdtC.js";
...
import { t as Input } from "./input-BrJF_8dN.js";
...
function FullBookImport() {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(FilePick, {
		...
		onFile: async (file) => {
			...
			if (!parsed.people.length && !parsed.attendance.length && !parsed.payments.length && !(parsed.expenses || []).length && !(parsed.policies || []).length && !(parsed.members || []).length) {
			...
			if (!confirm(`导入「${file.name}」？\n\n人员 ${parsed.people.length} 人 · 考勤 ${parsed.attendance.length} 条 · 发放 ${parsed.payments.length} 条（跳过 ${payMerge.skipped} 条重复）· 报销 ${(parsed.expenses || []).length} 条（跳过 ${expMerge.skipped} 条重复）· 保单 ${(parsed.policies || []).length} 份 · 保险人员 ${(parsed.members || []).length} 人。\n\n同名人员、同保单号保单、重复考勤（姓名+年月）与重复发放/报销会被跳过；现有数据追加保留。` + (legacyDup ? `\n\n${legacyDup}` : ""))) return;
			const byName = Object.fromEntries(store.people.map((p) => [p.name, p]));
			...
			toast.success(`整本导入完成：人员新增 ${added}，考勤 ${parsed.attendance.length} 条，发放新增 ${payMerge.added} 条（跳过 ${payMerge.skipped} 条重复），报销新增 ${expMerge.added} 条（跳过 ${expMerge.skipped} 条重复），保单 ${polAdded} 份，保险人员 ${memAdded} 人`);
		}
	});
}
