import { s as uid } from "./utils-BRICDxN_.js";
import { a as xlsx_exports, t as readSync } from "./xlsx-B0OoRtYU.js";
import { M as paymentsInYear, P as hasContent, S as parseNumber, T as daysBetween, _ as parseIdCard, a as normalizeContractStatus, c as splitTax, d as monthPay, h as normalizeIdDate, j as parseDateYmd, o as normalizeEntry, r as contractRollup, s as splitLegacyReceipts, u as getWageAt, x as parseNum, y as numOr } from "./contracts-BxZHWnH3.js";
...
function planAttendanceImport(rows, existing, targetYear, targetMonth, keepMonths) {
	return rows.map((row) => {
		...
		return {
			...
			year,
			month,
			conflict: existing.some((a) => a.year === year && a.month === month && a.name === row.name)
		};
	});
}
var titleStyle = () => ({
...
function buildFullWorkbook(args) {
	...
	for (const y of yearSet) {
		...
		const workers = people.filter((p) => attendance.some((a) => a.year === y && a.name === p.name && hasAttContent(a)));
		workers.forEach((p, i) => {
			const months = [];
			for (let m = 1; m <= 12; m++) {
