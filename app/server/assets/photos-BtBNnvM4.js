import { g as logOp } from "./audit-diff-BCw9WhZ-.js";
...
async function setPhoto(name, kind, dataUrl) {
	...
	logOp("上传照片", `${name} ${PHOTO_KIND_LABEL[kind] || kind}`, "照片");
}
async function deletePhoto(name, kind) {
	...
	logOp("删除照片", `${name} ${PHOTO_KIND_LABEL[kind] || kind}`, "照片");
}
