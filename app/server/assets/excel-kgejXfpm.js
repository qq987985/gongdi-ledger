...
import { a as xlsx_exports, t as readSync } from "./xlsx-B0OoRtYU.js";
import { a as round2, g as paymentsInYear, h as parseDateYmd, l as daysBetween, n as getWageAt, r as monthPay, v as hasContent } from "./wage-BVBIWt51.js";
import { _ as numOr, a as normalizeContractStatus, b as parseNumber, c as splitTax, d as parseIdCard, g as receiverOf, h as ownerKey, l as normalizeIdDate, m as nameKey, o as normalizeEntry, r as contractRollup, s as splitLegacyReceipts, y as parseNum } from "./contracts-FU2UPdFm.js";
const { utils } = xlsx_exports;
...
var NON_DATE_MARK = /^(在保|是|有|无|否|长期|无期限|--?|—|\/|n\/a|na|none)$/i;
function normalizeEndDate(s) {
	const t = (s || "").trim();
...
function parseContractFiles(raw) {
	return String(raw || "").split(/[、;；,，\n]/).map((x) => x.trim()).filter(isSafeFileName).map((fileName) => ({
		id: uid(),
		fileName
	}));
}
function contractFilesCell(list) {
	return (list || []).map((c) => (c?.fileName || "").trim()).filter(isSafeFileName).join("、");
}
function keyDate(s) {
	const t = (s || "").trim();
	if (!t) return "";
...
function findDuplicateGroups(list, keyOf) {
	...
	const out = [];
	for (const [key, items] of byKey) if (items.length > 1) out.push({
		key,
		items
	});
	return out;
}
function findDuplicatePayments(existing) {
	return findDuplicateGroups(existing, paymentKey);
}
function duplicateNotice(groups, what) {
	if (!groups.length) return "";
	const rows = groups.reduce((s, g) => s + g.items.length, 0);
	return `注意：本台账现有${what}里有 ${groups.length} 组重复（同人、同日期、同金额，共 ${rows} 条）。本次导入不会再新增重复；但这些历史重复会让合计偏大，请到对应页面核对后手动删除 —— 系统不会自动删你的数据。`;
}
...
function planAttendanceImport(rows, existing, targetYear, targetMonth, keepMonths) {
	return rows.map((row) => {
		...
		return {
			...
			year,
			month,
			conflict: existing.some((a) => a.year === year && a.month === month && nameKey(a.name) === nameKey(row.name))
		};
	});
}
var titleStyle = () => ({
...
function parseWageHistoryCell(raw) {
	const t = (raw || "").trim();
	if (!t || t === "[]") return [];
	let arr;
	try {
...
function rowToPerson(row) {
	...
	return {
		...
		mealAllowance: parseNumber(pick(row, [
			"餐补/天",
			"餐补",
			"mealAllowance"
		])),
		wageHistory: parseWageHistoryCell(pick(row, [
			"工资历史",
			"调薪历史",
			"wageHistory"
		])),
...
function rowToPayment(row) {
	...
	return {
		...
		amount: round2(parseNumber(pick(row, [
			"发放金额(元)",
			"发放金额",
			"金额"
		]))),
...
function paymentSheetAoa(payments) {
	...
	[...payments].sort((a, b) => (a.date || "9").localeCompare(b.date || "9")).forEach((p, i) => {
		payAoa.push([
			i + 1,
			p.owner,
			p.date,
...
function rowToExpense(row, fallbackYear) {
	...
	const amount = amt.n;
	const year = parseNumber(pick(row, ["年份"])) || fallbackYear || (/* @__PURE__ */ new Date()).getFullYear();
...
function insuranceMemberTemplateWb() {
	const wb = utils.book_new();
	utils.book_append_sheet(wb, titledSheet("保险人员导入模板", [
		[
			"姓名",
			"队长",
			"备注"
...
function parseInsuranceMembersSheet(buf) {
	const wb = readWb(buf);
	const preferred = wb.SheetNames.find((n) => n.includes("保险")) || wb.SheetNames[0];
	return sheetRecords(wb.Sheets[preferred]).map((row) => {
		const name = pick(row, ["姓名", "name"]);
		if (!name || isTotalRow(name)) return null;
		return {
			id: uid(),
			...
			startDate: normalizeDate(pick(row, [
				"开始日期",
				"开始时间",
				"日期"
			])),
...
function parseContractWorkbook(buf) {
	...
	const isEntrySheet = (n) => /报量|开票|收款/.test(n) && !n.includes("合同");
	const sheets = wb.SheetNames.filter((n) => !n.includes("填写说明") && !isDerivedSheet(n));
	const lookup = (project, year, code) => [...byKey.values()].find((x) => x.name === project && (!year || x.year === year) && (!code || x.code === code)) || contracts.find((x) => x.name === project);
	for (const name of sheets) {
		...
		for (const row of sheetRecords(wb.Sheets[name])) {
			...
			const year = parseNumber(pick(row, ["年份"])) || (/* @__PURE__ */ new Date()).getFullYear();
			...
			const reason = ["无合同原因", "没有合同原因"].some((k) => k in row) ? pick(row, ["无合同原因", "没有合同原因"]) : "";
			const c = {
				id: uid(),
				year,
				code,
				...
				remark: pick(row, ["备注"]),
				hasPaper: paperCol ? contractHasPaper(pick(row, [
					"有无合同",
					"合同原件",
					"有无合同原件"
				])) : true,
				noContractReason: reason,
				scanFileName: pick(row, [
					"合同扫描件",
					"扫描件",
					"合同电子版"
				])
	...
	const hasDetail = (kw) => wb.SheetNames.some((n) => !n.includes("合同") && n.includes(kw));
	for (const name of sheets) {
		...
		for (const row of sheetRecords(wb.Sheets[name])) {
			...
			if (entrySheet || kindRaw) {
				...
				const project$1 = pick(row, ["项目名称", "项目"]);
				if (!project$1 || isTotalRow(project$1)) continue;
				const c$1 = lookup(project$1, numOr(pick(row, ["年份"]), 0), pick(row, ["项目号"]));
				if (!c$1) continue;
				...
				entries.push(normalizeEntry({
					...
					taxRate: taxCol ? parsePct(pick(row, ["开票税率", "税率"])) : kind === "invoice" ? c$1.taxRate : 0,
			...
			const project = pick(row, ["项目名称"]);
			if (!project || isTotalRow(project)) continue;
			const c = lookup(project, numOr(pick(row, ["年份"]), 0), pick(row, ["项目号"]));
			if (!c) continue;
			const report = numPick(row, [
				"月报量金额",
				"月报量",
				"报量金额"
			]);
			const invoice = numPick(row, ["开票金额"]);
			const receipt = numPick(row, [
				"收款账金额",
				"收款金额",
				"已付（代付+到分包）",
				"已付(代付+到分包)"
			]);
			if (report && !hasDetail("报量")) entries.push(normalizeEntry({
				contractId: c.id,
				kind: "report",
				date: `${c.year}-01-31`,
				amount: report,
			...
			if (invoice && !hasDetail("开票")) entries.push(normalizeEntry({
				contractId: c.id,
				kind: "invoice",
				date: `${c.year}-01-31`,
				amount: invoice,
			...
			if (receipt && !hasDetail("收款")) entries.push(normalizeEntry({
				contractId: c.id,
				kind: "receipt",
				date: `${c.year}-01-31`,
				amount: receipt,
...
function mergeContracts(existing, existingEntries, incoming, incomingEntries) {
...
function contractTemplateWb() {
	...
	utils.book_append_sheet(wb, noteSheet([
		...
		"「有无合同」填 有/无；填 无 时请在「无合同原因」写清原因（两个字段与导出合同表一一对应）。",
		"「合同扫描件」填文件名字符串（一般留空，在软件里上传更稳妥）；有文件名即算有合同。",
...
function parseFullAttendanceWorkbook(buf, fallbackYear) {
	...
	if (polName) {
		const aoa = utils.sheet_to_json(wb.Sheets[polName], {
			header: 1,
			defval: "",
			raw: true
		});
		...
		if (hi >= 0) {
			...
			for (const row of rows) {
				...
				policies.push({
					...
					linkedPolicyId: "",
					contracts: parseContractFiles(pick(row, [
						"合同文件",
						"保险合同",
						"保单文件"
					])),
			...
			if (memName) for (const row of sheetRecords(wb.Sheets[memName])) {
				const name = pick(row, ["姓名"]);
...
function buildFullWorkbook(args) {
	...
	for (const { year: y, month: m } of monthList) {
		const monthRows = attendance.filter((a) => a.year === y && a.month === m && a.name.trim() && hasAttContent(a));
		const aoa = [[`${y}年${m}月考勤`], [
			"序号",
			"姓名",
			"班组",
		...
		monthRows.forEach((a, i) => {
			...
			aoa.push([
				...
				calc.deduction || "",
				calc.meal || "",
				wage.payType === "month" ? "按月" : "按工天",
				wage.payType === "month" ? wage.monthWage || "" : wage.dailyWage || "",
				calc.ot || "",
				calc.pay || "",
				wage.otRule || "",
				a.remark || ""
	...
	if (insurancePolicies.length) {
		...
		for (const p of insurancePolicies) {
			...
			paoa.push([
				...
				dpart(p.periodStart),
				dpart(p.periodEnd),
				daysBetween(p.periodStart, p.periodEnd) || "",
				(p.premiumPerPerson || 0) * (p.headcount || 0) || "",
				linked?.policyNo || "",
				contractFilesCell(p.contracts),
				p.remark
			]);
		}
	...
	if (contracts.length || contractEntries.length) {
		const cwb = buildContractWorkbook({
			contracts,
	...
	for (const y of yearSet) {
		...
		const workers = people.filter((p) => attendance.some((a) => a.year === y && nameKey(a.name) === nameKey(p.name) && hasAttContent(a)));
		workers.forEach((p, i) => {
			const months = [];
			for (let m = 1; m <= 12; m++) {
...
export { pick as $, rowToPerson as A, isSafeFileName as B, attendanceTemplateWb as C, parsePeopleSheet as D, buildPeopleWorkbook as E, duplicateNotice as F, normalizeDate as G, mergeExpenses as H, expenseKey as I, numCell as J, normalizeEndDate as K, findDuplicateGroups as L, cellStr as M, contractFilesCell as N, peopleSheetAoa as O, detectWorkbookYear as P, paymentKey as Q, findDuplicatePayments as R, attFromRow as S, DEMO_PEOPLE as T, mergePayments as U, isTotalRow as V, mergeUnique as W, numPick as X, numOut as Y, parseContractFiles as Z, parsePaymentSheet as _, contractTemplateWb as a, titledSheet as at, rowToPayment as b, insuranceMemberTemplateWb as c, expenseSheetAoa as d, planAttendanceImport as et, expenseTemplateWb as f, buildPaymentWorkbook as g, DEMO_PAY as h, contractKey as i, sheetToRows as it, SKIP_SHEETS as j, peopleTemplateWb as k, parseInsuranceMembersSheet as l, rowToExpense as m, parseFullAttendanceWorkbook as n, sheetFromAoa as nt, mergeContracts as o, utils as ot, parseExpenseSheet as p, noteSheet as q, buildContractWorkbook as r, sheetRecords as rt, parseContractWorkbook as s, buildFullWorkbook as t, readWb as tt, buildExpenseWorkbook as u, paymentSheetAoa as v, parseAttendanceSheet as w, DEMO_ATT as x, paymentTemplateWb as y, isDerivedSheet as z };
