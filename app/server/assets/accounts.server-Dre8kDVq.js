import { S as writeBookMeta, a as listBookIds, h as runWithBook, l as readBookMeta, n as dataDir, o as persistOn, t as appendAudit } from "./nas-fs.server-y_Ak6PUm.js";
import { i as PRESETS, o as canWriteLedger, s as hasPerm, t as ALL_PERMS } from "./perms-C-hDVhes.js";
...
async function handleAuthPost(request) {
	...
	if (op === "verify") {
		...
		if (!user || user.hash !== await hashPassword(password)) return Response.json({ error: "密码错误" }, { status: 401 });
		if (user.disabled) return Response.json({ error: "账户已停用" }, { status: 403 });
	...
	if (op === "deleteBook") {
		...
		await writeFileShape(data);
		const { removeBookDir } = await import("./nas-fs.server-DUdNYRlo.js");
