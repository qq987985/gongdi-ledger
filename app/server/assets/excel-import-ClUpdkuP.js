...
import { C as parseFullAttendanceWorkbook, E as parsePeopleSheet, S as parseExpenseSheet, T as parsePaymentSheet, b as parseAttendanceSheet, w as parseInsuranceMembersSheet, x as parseContractWorkbook } from "./excel-YBHp7SId.js";
...
function FullBookImport() {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(FilePick, {
		...
		onFile: async (file) => {
			...
			if (!confirm(`导入「${file.name}」？\n\n人员 ${parsed.people.length} 人 · 考勤 ${parsed.attendance.length} 条 · 发放 ${parsed.payments.length} 条 · 报销 ${(parsed.expenses || []).length} 条 · 保单 ${(parsed.policies || []).length} 份 · 保险人员 ${(parsed.members || []).length} 人。\n\n同名人员、同保单号保单与重复考勤（姓名+年月）会被跳过；现有数据追加保留。`)) return;
			const byName = Object.fromEntries(store.people.map((p) => [p.name, p]));
			...
			toast.success(`整本导入完成：人员新增 ${added}，考勤 ${parsed.attendance.length} 条，发放 ${parsed.payments.length} 条，报销 ${(parsed.expenses || []).length} 条，保单 ${polAdded} 份，保险人员 ${memAdded} 人`);
		}
	});
}
