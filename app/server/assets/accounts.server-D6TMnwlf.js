import { S as writeBookMeta, a as listBookIds, h as runWithBook, l as readBookMeta, n as dataDir, o as persistOn, t as appendAudit } from "./nas-fs.server-DVmz8Fj_.js";
import { c as hasPerm, i as PRESETS, o as canManageLedger, s as canWriteLedger, t as ALL_PERMS } from "./perms-DxXw_WvE.js";
...
async function handleAuthPost(request) {
	...
	if (op === "deleteBook") {
		...
		await writeFileShape(data);
		const { removeBookDir } = await import("./nas-fs.server-WE3wGoHQ.js");
...
async function withTenant(request, fn, need) {
	...
	if (need === "ledger.manage" && !canManageLedger(t.perms)) return Response.json({ error: "没有修改整本台账的权限" }, { status: 403 });
	if (need && need !== "ledger.write" && need !== "ledger.manage" && !hasPerm(t.perms, need)) return Response.json({ error: "没有权限" }, { status: 403 });
