...
async function seedTemplates() {
	...
	try {
		const excel = await import("./excel-hix9QRxH.js");
...
async function ledgerRevision() {
	return ledgerRevisionValue(await readLedger());
}
var ledgerWriteQueue = Promise.resolve(true);
async function writeLedgerNow(data, expectedRevision) {
	if (!persistOn()) return true;
	await ensureDirs();
	if (expectedRevision !== void 0 && await ledgerRevision() !== expectedRevision) return false;
	const p = ledgerPath();
...
function writeLedger(data, expectedRevision) {
	ledgerWriteQueue = ledgerWriteQueue.then(() => writeLedgerNow(data, expectedRevision), () => writeLedgerNow(data, expectedRevision));
...
export { scanPhotoFolder as C, logServer as D, writeLedger as E, savePhoto as S, writeBookMeta as T, removePhoto as _, ledgerRevision as a, saveBackup as b, listBookIds as c, readAudit as d, readBookMeta as f, removeDocFile as g, removeBookDir as h, findPhotoPath as i, persistOn as l, readVersionText as m, dataDir as n, ledgerRevisionOf as o, readLedger as p, findDoc as r, ledgerRevisionValue as s, appendAudit as t, photoFlags as u, runWithBook as v, writeAudit as w, saveDoc as x, safeBookId as y };
