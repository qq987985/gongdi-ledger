...
import { i as parseChangelog, t as AppShell } from "./shell-BGKCyme_.js";
...
import { S as writeAudit, a as ledgerRevision, b as savePhoto, c as photoFlags, d as readLedger, f as readVersionText, h as removePhoto, i as findPhotoPath, l as readAudit, m as removeDocFile, r as findDoc, s as persistOn, t as appendAudit, v as saveBackup, w as writeLedger, x as scanPhotoFolder, y as saveDoc } from "./nas-fs.server-Daz2tm8t.js";
import "./perms-DxXw_WvE.js";
import { a as resolveTenant, n as handleAuthPost, o as withTenant, t as ensureAccounts } from "./accounts.server-C4JS1wHu.js";
import { C as hasWork, p as dateYear, y as parseDateYmd } from "./contracts-DonCYL5F.js";
import { D as paymentTemplateWb, O as peopleTemplateWb, c as buildExpenseWorkbook, d as buildPeopleWorkbook, g as insuranceMemberTemplateWb, h as expenseTemplateWb, l as buildFullWorkbook, o as attendanceTemplateWb, p as contractTemplateWb, s as buildContractWorkbook, u as buildPaymentWorkbook } from "./excel-YBHp7SId.js";
import { t as useApp } from "./store-CEZ_o1JK.js";
import { t as Toaster } from "./dist-2eP1hwXK.js";
...
const Route$16 = createFileRoute("/api/auth")({ server: { handlers: {
	GET: async ({ request }) => {
		...
		const { resolveTenant: resolveTenant$1, publicUser, memberList } = await import("./accounts.server-C1MshA4_.js");
		const { hasPerm } = await import("./perms-DCCan3qx.js");
...
const Route$20 = createFileRoute("/api/ledger")({ server: { handlers: {
	...
	PUT: async ({ request }) => {
		...
		return withTenant(request, async () => {
			...
			if (!await writeLedger(body, expected === null ? void 0 : expected)) return Response.json({
...
const Route$22 = createFileRoute("/api/photo-file")({ server: { handlers: { GET: async ({ request }) => {
...
const Route$23 = createFileRoute("/api/photo-flags")({ server: { handlers: {
	GET: async ({ request }) => {
...
const Route$24 = createFileRoute("/api/photo-scan")({ server: { handlers: { POST: async ({ request }) => {
...
const Route$25 = createFileRoute("/api/update")({ server: { handlers: {
	GET: async ({ request }) => {
		try {
			const t = await resolveTenant(request);
	...
	POST: async ({ request }) => {
		try {
			...
			if (!sameOrigin(request)) return Response.json({ error: "来源不一致，已拒绝" }, { status: 403 });
			const t = await resolveTenant(request);
...
const Route$26 = createFileRoute("/api/version")({ server: { handlers: { GET: async () => {
	const text = await readVersionText();
...
var rootRouteChildren = {
	...
	ApiPhotoFileRoute: Route$22.update({
		id: "/api/photo-file",
		path: "/api/photo-file",
		getParentRoute: () => Route
	}),
	ApiPhotoFlagsRoute: Route$23.update({
		id: "/api/photo-flags",
		path: "/api/photo-flags",
		getParentRoute: () => Route
	}),
	ApiPhotoScanRoute: Route$24.update({
		id: "/api/photo-scan",
		path: "/api/photo-scan",
		getParentRoute: () => Route
	}),
	ApiUpdateRoute: Route$25.update({
		id: "/api/update",
		path: "/api/update",
		getParentRoute: () => Route
	}),
	ApiVersionRoute: Route$26.update({
		id: "/api/version",
		path: "/api/version",
		getParentRoute: () => Route
	}),
