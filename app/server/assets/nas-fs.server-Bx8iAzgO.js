...
var ensuredFor = /* @__PURE__ */ new Set();
var initializing = /* @__PURE__ */ new Map();
function dataDir() {
...
function safeBookId(id) {
	...
	if (safe === "." || safe === ".." || /[\x00-\x1f\x7f]/.test(id)) throw new Error("台账编号不合法");
	return safe;
}
...
async function seedTemplates() {
	...
	try {
		const excel = await import("./excel-C5QWrU23.js");
...
var auditBrokenByPath = /* @__PURE__ */ new Map();
async function readAuditSnapshot() {
	...
	if (existsSync(p)) try {
		return {
			entries: parseAuditFile(JSON.parse(await readFile(p, "utf8"))),
			unreadable: false
		};
	} catch (err) {
	...
	for (const legacy of legacyAuditPaths()) {
		...
		try {
			...
			await logServer("error", "历史操作记录文件读取失败", {
				path: legacy,
				error: String(err)
			});
			return {
...
async function writeAuditNow(entries) {
...
function withAuditTransaction(fn) {
...
export { savePhoto as A, findPhotoPath as C, removeDocFile as D, photoNameWritable as E, runWithBook as F, logServer as I, DOC_CN as M, dataDir as N, removePhoto as O, persistOn as P, findDoc as S, photoFlags as T, writeBookMeta as _, ledgerRevisionOf as a, adoptLegacyAssets as b, listBookIds as c, readBookMeta as d, readLedger as f, withAuditTransaction as g, saveBackup as h, isManagedBackupFile as i, scanPhotoFolder as j, saveDoc as k, pruneBackups as l, removeBookDir as m, appendAudit as n, ledgerRevisionValue as o, readVersionText as p, backupKeepCount as r, ledgerUnreadable as s, DEFAULT_BACKUP_KEEP as t, readAudit as u, writeLedger as v, isWritablePhotoDataUrl as w, docIdWritable as x, writeLedgerEx as y };
