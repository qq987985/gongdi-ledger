import { F as require_react, V as __toESM, t as require_react_dom } from "../server.js";
import { a as money } from "./utils-DqsA5Dz5.js";
import { a as round2, h as parseDateYmd, o as wageLabel } from "./wage-BVBIWt51.js";
...
function fmtDateOrEmpty(v) {
	const raw = String(v ?? "").trim();
	if (!raw) return "（空）";
	return parseDateYmd(raw) || fmtText(raw);
}
function fmtPayType(v) {
...
function fmtSensitive(v) {
	return String(v ?? "").trim() ? "已填（值不记录）" : "（空）";
}
function fmtHistory(v) {
...
function compareFields(before, after, spec) {
	const out = [];
	for (const f of spec) {
...
function diffNote(dropped) {
	return `…（另有 ${dropped} 项）`;
}
function formatChanges(changes, maxChars = 300) {
	...
	for (let i = 0; i < changes.length; i += 1) {
		...
		const budget = maxChars - (rest > 0 ? diffNote(rest).length + 1 : 0);
		if (parts.length && used + part.length + 1 > budget) return parts.join("；") + "；" + diffNote(changes.length - parts.length);
...
function diffDetail(summary, changes, maxChars = 300) {
...
function detailPlain(detail) {
	const { summary, changes } = parseDetail(detail);
	if (!changes.length) return summary;
	return `${summary}｜${changes.map((c) => `${c.label} ${c.before}→${c.after}`).join("；")}`;
}
const PERSON_SPEC = [
	{
		key: "name",
...
var asRow = (v) => v;
function personChanges(before, after) {
...
function expenseChanges(before, after) {
	return compareFields(asRow(before), asRow(after), EXPENSE_SPEC);
}
function prefixChanges(prefix, changes) {
...
function batchChanges(pairs, changesOf, nameOf) {
	const out = [];
	for (const { before, after } of pairs) out.push(...prefixChanges(nameOf(before), changesOf(before, after)));
	return out;
}
function deletedChanges(rows, labelOf, valueOf) {
	return rows.map((row) => ({
		label: labelOf(row),
		before: valueOf(row),
		after: "已删除"
...
function paymentDeletedChanges(rows) {
	return deletedChanges(rows, (p) => `${fmtText(p.owner)} ${p.date ? fmtDate(p.date) : "待发放"}`, (p) => `¥${fmtMoney(p.amount)}`);
}
function personDeletedChanges(rows) {
	return deletedChanges(rows, (p) => fmtText(p.name), (p) => `${p.team || "未分班组"} ${wageLabel(p)}`);
}
function expenseDeletedChanges(rows) {
	return deletedChanges(rows, (e) => fmtText(e.name), (e) => `${e.claimant || "未填报销人"} ¥${fmtMoney(e.amount)} ${e.status || ""}`.trim());
}
var ATT_SPEC = [
	{
		key: "days",
...
function digestAttendance(rows) {
...
function attendanceChanges(beforeRows, afterRows) {
...
export { Toaster as _, diffDetail as a, expenseDeletedChanges as c, paymentChanges as d, paymentDeletedChanges as f, logOp as g, fetchAudit as h, detailPlain as i, fmtMoney as l, personDeletedChanges as m, batchChanges as n, entryChanges as o, personChanges as p, contractChanges as r, expenseChanges as s, attendanceChanges as t, parseDetail as u, toast as v };
