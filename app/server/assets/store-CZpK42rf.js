...
import { A as derivedYears, M as localToday, P as nextYear, _ as parseIdCard, b as nameKey, h as normalizeIdDate, o as normalizeEntry, s as splitLegacyReceipts, w as numOrWarn } from "./contracts-CNGvTFF_.js";
...
const useApp = create()(persist((set, get) => ({
	...
	upsertPerson: (p) => {
		...
		if (i >= 0) {
			...
			logOp(plan.oldName === plan.newName ? "修改人员" : "人员改名", plan.oldName === plan.newName ? nextP.name : renameLogDetail(plan), "人员");
		} else {
			set({ people: [...people, {
				...nextP,
				id: pid || uid()
