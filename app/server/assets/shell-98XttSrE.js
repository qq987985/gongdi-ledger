...
import { n as nasEnabled, r as pullNasLedger } from "./nas-sync-YEjyDlzb.js";
...
function useHydrateStore() {
	import_react.useEffect(() => {
		(async () => {
			...
			try {
				const { startNasSync } = await import("./nas-sync-CF7VjliL.js");
				await startNasSync();
			} catch {}
	...
	async function refreshGate() {
		try {
			...
			if (!s.persist) setGate("app");
				...
				setGate("app");
				try {
					const { detectNas, pullNasLedger: pullNasLedger$1 } = await import("./nas-sync-CF7VjliL.js");
					await detectNas();
