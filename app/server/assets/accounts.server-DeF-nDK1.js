import { S as writeBookMeta, a as listBookIds, h as runWithBook, l as readBookMeta, n as dataDir, o as persistOn, t as appendAudit } from "./nas-fs.server-BF3NZS-m.js";
...
function scryptHash(raw) {
	...
	return `scrypt$16384$8$1$${salt}$${scryptSync(raw, salt, 64, {
		N: 16384,
		r: 8,
		p: 1
	}).toString("hex")}`;
...
async function verifyStoredHash(stored, raw) {
	...
	if (stored.startsWith("scrypt$")) {
		...
		try {
			const calc = scryptSync(raw, salt, 64, {
				N: Number(n),
				r: Number(r),
				p: Number(p)
			});
...
async function handleAuthPost(request) {
	...
	if (op === "deleteBook") {
		...
		await writeFileShape(data);
		const { removeBookDir } = await import("./nas-fs.server-BorXpEYx.js");
