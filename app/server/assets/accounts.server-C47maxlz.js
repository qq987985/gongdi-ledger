import { F as persistOn, I as runWithBook, L as logServer, P as dataDir, f as readBookMeta, l as listBookIds, n as appendAudit, v as writeBookMeta } from "./nas-fs.server-CK9TA_7Z.js";
import { c as hasPerm, i as PRESETS, o as canManageLedger, s as canWriteLedger, t as ALL_PERMS } from "./perms-D5IsG1Md.js";
import { s as uid } from "./utils-DqsA5Dz5.js";
import { existsSync } from "node:fs";
import { join } from "node:path";
import { mkdir, readFile, rename, rm, writeFile } from "node:fs/promises";
import { createHash, randomBytes, scryptSync, timingSafeEqual } from "node:crypto";
function maxOwnedBooks(env = typeof process !== "undefined" ? process.env : {}) {
	const raw = Number(env?.MAX_OWNED_BOOKS);
	if (!Number.isFinite(raw) || raw < 1) return 5;
	return Math.min(Math.floor(raw), 1e3);
}
function ownedBookCount(books, userId) {
	return books.filter((b) => Boolean(userId) && b.ownerId === userId).length;
}
function ownedBooksLimitMessage(limit) {
...
function scryptHash(raw) {
	const t = raw.trim();
	if (!t) return "";
	const salt = randomBytes(16).toString("hex");
	return `scrypt$16384$8$1$${salt}$${scryptSync(t, salt, 64, {
		N: 16384,
		r: 8,
		p: 1
	}).toString("hex")}`;
}
async function verifyStoredHash(stored, raw) {
	const t = raw.trim();
	...
	if (stored.startsWith("scrypt$")) {
		...
		try {
			const calc = scryptSync(t, salt, 64, {
				N: Number(n),
				r: Number(r),
				p: Number(p)
			});
...
var authRate = /* @__PURE__ */ new Map();
var AUTH_RATE_SWEEP_AT = 500;
function rateSweep() {
	const now = Date.now();
	for (const [k, rec] of authRate) if (rec.until <= now && rec.fails === 0) authRate.delete(k);
}
function rateKey(kind, username) {
	return `${kind}:${username.toLowerCase()}`;
}
function rateLocked(kind, username) {
	const rec = authRate.get(rateKey(kind, username));
	return Boolean(rec && rec.until > Date.now());
}
function rateFail(kind, username) {
	if (authRate.size >= AUTH_RATE_SWEEP_AT) rateSweep();
	const k = rateKey(kind, username);
	const rec = authRate.get(k) || {
		fails: 0,
		until: 0
	};
	rec.fails += 1;
	if (rec.fails >= 5) {
		rec.until = Date.now() + 5 * 6e4;
		rec.fails = 0;
	}
	authRate.set(k, rec);
}
function rateOk(kind, username) {
	authRate.delete(rateKey(kind, username));
}
var accountsBroken = false;
function accountsUnreadable() {
	return accountsBroken;
}
const ACCOUNTS_BROKEN_MSG = "服务器上的账户数据（data/accounts/accounts.json）读取失败，已停止自动修复与初始化，避免覆盖现有账号。请从备份恢复该文件。";
async function readFileShape() {
	...
	for (const p of accountsPathCandidates()) {
		...
		try {
			...
			accountsBroken = true;
			await logServer("error", "账户库读取失败：已停止自动修复", {
				path: p,
				error: String(err)
			});
			return {
...
var shapeQueue = Promise.resolve();
async function writeFileShapeNow(data) {
	if (!persistOn()) return;
...
async function ensureAccounts() {
	...
	if (accountsBroken) return {
		users: [],
		books: []
	};
	let salted = false;
	for (const u of data.users) if (!u.tokenSalt) {
...
async function logLegacyAccessHashNotice() {
	const root = dataDir();
	if (!root) return;
	for (const f of [join(root, "books", "default", "ledger.json"), join(root, "ledger.json")]) {
		if (!existsSync(f)) continue;
		try {
			if (JSON.parse(await readFile(f, "utf8"))?.accessHash) await logServer("warn", "检测到旧版台账里的 accessHash：已不再自动创建管理员", {
				path: f,
...
async function auditTenantDelete(t, row) {
	try {
		await appendAudit({
			userId: t.user?.id || "",
		...
		await logServer("error", "删除操作留痕失败", {
			action: row.action,
			error: String(err)
		});
	}
...
async function handleAuthPost(request) {
	...
	if (op === "setup") {
		...
		if (!username || password.length < 8) return Response.json({ error: "用户名必填，密码至少 8 位" }, { status: 400 });
		const user = {
			id: "admin",
			username,
			name,
	...
	if (op === "login") {
		...
		const password = (body.password || "").trim();
		if (rateLocked("login", username)) return Response.json({ error: "尝试次数太多，请 5 分钟后再试" }, { status: 429 });
	...
	if (op === "verify") {
		const password = (body.password || "").trim();
		const userId = cookies(request).gongdi_u || "";
		const user = data.users.find((u) => u.id === userId);
		if (rateLocked("verify", userId)) return Response.json({ error: "尝试次数太多，请 5 分钟后再试" }, { status: 429 });
		if (!user || !await verifyStoredHash(user.hash, password)) {
	...
	if (op === "createBook") {
		...
		if (me.role !== "admin") {
			...
			if (owned >= limit) return Response.json({
				error: ownedBooksLimitMessage(limit),
				limit,
				owned
			}, { status: 400 });
		}
	...
	if (op === "deleteBook") {
		...
		await writeFileShape(data);
		const { removeBookDir } = await import("./nas-fs.server-DCrh-EUQ.js");
	...
	if (op === "changePassword") {
		if (!await verifyStoredHash(me.hash, body.old || "")) return Response.json({ error: "当前密码不对" }, { status: 400 });
		if ((body.password || "").trim().length < 8) return Response.json({ error: "新密码至少 8 位" }, { status: 400 });
	...
	if (op === "createUser") {
		...
		if (book && user.id !== book.ownerId) {
			const explicit = String(body.perms || "").split(/[,\s]+/).map((x) => x.trim()).filter((x) => x === "*" || ALL_PERMS.includes(x));
	...
	if (op === "addMember" || op === "setMember") {
		...
		if (!(me.role === "admin" || book.ownerId === me.id)) {
			if (target.id === me.id) return Response.json({ error: "不能修改自己的权限" }, { status: 403 });
			const mine = new Set(permsOf(me, book));
			if (!mine.has("*")) {
				const over = (perms.includes("*") ? ALL_PERMS : perms).filter((p) => !mine.has(p));
				if (over.length) return Response.json({ error: `不能授予自己没有的权限：${over.slice(0, 4).join("、")}` }, { status: 403 });
			}
		}
...
async function gateTenant(request, need) {
	if (!persistOn()) return {};
	const t = await resolveTenant(request);
	if (t.broken) return Response.json({
		error: ACCOUNTS_BROKEN_MSG,
		broken: true
	}, { status: 503 });
	...
	if (!t.bookId) {
		if (t.bookDenied) return Response.json({
...
async function permReject(request, t, need) {
	const needs = need === void 0 ? [] : Array.isArray(need) ? need : [need];
...
function needDenied(request, t, need) {
	return permReject(request, t, need);
}
function runInTenant(t, fn) {
...
export { gateTenant as a, needDenied as c, runInTenant as d, withTenant as f, ensureAccounts as i, publicUser as l, accountsUnreadable as n, handleAuthPost as o, auditTenantDelete as r, memberList as s, ACCOUNTS_BROKEN_MSG as t, resolveTenant as u };
