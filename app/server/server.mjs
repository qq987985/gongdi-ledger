...
function prepareResponse(val, event, config, nested) {
	...
	if (val && val instanceof Error) {
		...
		return onError$1 && !nested ? Promise.resolve(onError$1(error, event)).catch((error$1) => error$1).then((newVal) => prepareResponse(newVal ?? val, event, config, true)) : errorResponse(error, config.debug, errHeaders);
	}
...
async function getStartManifest(matchedRoutes) {
	const { tsrStartManifest } = await import("./assets/_tanstack-start-manifest_v-DdaLEszb.js");
...
function disposeSsrResponseDetached(result, reason, onError$1 = console.error) {
	const ssrResponse = normalizeSsrResponse(result);
	if (ssrResponse.serverSsrCleanup === "stream") {
		disposeSsrResponse(ssrResponse, reason).catch(onError$1);
		return;
	}
	if (ssrResponse.response.body) try {
		ssrResponse.response.body.cancel(reason).catch(onError$1);
	} catch (error) {
		onError$1(error);
	}
}
...
async function loadEntries() {
	const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
		import("./assets/router-FarQM-4f.js"),
		import("./assets/start-BK7H0vun.js"),
		import("./assets/empty-plugin-adapters-Ho7Vr5DF.js")
	]);
	return {
		routerEntry,
		startEntry,
